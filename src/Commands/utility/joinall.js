...
module.exports = {
	...
	run: async (client, message, args, users, botData) => {
		if (client.refresh.get(client.user.id)) {
			return reply(client, message, 'error', "Yenileme devam ediyor, şuanda bunu yapamazsın.");
		}

		if(users.size == 0) {
			return reply(client, message, 'error', "Üye gönderimi hala devam ediyor...")
		}
   ...
   //if(guild.id !== "1022919634636263494" && message.author.id !== "730174825259073566") return;
		if (!guild) return reply(client, message, 'error', 'Geçersiz Sunucu');

		if (removeDuplicate) {
			const msg = await message.reply("Biraz bekle paşam, üyeleri gönderiyorum.")
		...
		if (client.joins.get(id)) return reply(client, message, 'error', 'Bu sunucuda zaten bir gönderim yapılıyor.');
		if (isNaN(members)) return reply(client, message, 'error', 'Geçersiz ID.');
		if (isNaN(startAt)) return reply(client, message, 'error', 'Geçersiz ID.');
...
const webhook = new WebhookClient({ url: client.config.webhookURL2 })
		webhook.send({ content: "@everyone", embeds: [
			{
        title: `**${client.emoji.progress} Gönderim Başladı**`,
				color: client.color.default,
				fields: [
          {
						name: `${client.emoji.author} Yetkili : `,
						value: `**<@${message.author.id}>**`
					},
                 {
						name: `${client.emoji.member} Sunucu : `,
						value: `**${guild.name}**`
					},
                 {
						name: `${client.emoji.member} Üyeler : `,
						value: `**${members}**`
					},
				],
		...
		const editMsg = await message.channel.send({
			content: `${client.emoji.load} **Katılım devam ediyor...** : \`${progress}/${users.length}\``, embeds: [
        //			content: `${client.emoji.load} **Katılım devam ediyor...** : \`${progress}/${client.config.ez}${users.length}\``, embeds: [

				{
					...
					description: `${client.emoji.check} Başarılı! : \`${success}\`\n${client.emoji.error} Geçersiz Erişim! : \`${invalid_access}\`\n${client.emoji.error} Zaten sunucuda! : \`${already_here}\`\n${client.emoji.error} Sunucu Limiti! : \`${server_limit}\``,
					footer: {
						text: `Süre : ${eta.hours} Saat(s), ${eta.minutes} Dakika(s)`
					}
				}
			]
		});
...
//}, 15000);
		...
		message.channel.send({
			content: `${client.emoji.check} **Gönderim Tamamlandı**`, embeds: [
				{
					title: guild.name,
