...
module.exports = {
	...
	run: async (client, message, args, users, botData) => {
		...
		if (client.refresh.get(client.user.id)) {
			return reply(client, message, 'error', "Yenileme zaten devam ediyor...");
		}
        ...
        const msg = await message.channel.send({
            content: `**${client.emoji.load} Yenileme devam ediyor...** : \`${progress}/${users.length}\``,
            embeds: [{
                description: `${client.emoji.check} Başarılı! : \`${succ}\`\n${client.emoji.error} Hata! : \`${error}\``,
        ...
        for (const user of users) {
            await refresh(user.refresh_token, user.id);
            progress++;
        ...
        await message.channel.send({
            content: `**${client.emoji.check} Yenileme Tamamlandı! **`,
            embeds: [{
                description: `${client.emoji.check} Başarılı! : \`${succ}\`\n${client.emoji.error} Hata! : \`${error}\``,
